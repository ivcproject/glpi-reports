...
(SELECT  GROUP_CONCAT(CONCAT(IFNULL(CONCAT(glpi_usertitles.name,"<br>"),""), realname," ", firstname," <br>", phone,"")) AS "Исполнитель" FROM glpi_tickets_users
 ...
 WHERE type=1 AND tickets_id=glpi_tickets.id ) AS initi, "" as software
FROM glpi_tickets 
...
WHERE glpi_changes.is_deleted=0 AND (glpi_changes.name not like "%тест%" and glpi_changes.name not like "%test%") ' +
